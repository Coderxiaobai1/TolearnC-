...
def ALL(path,m):
    ...
    Phi = abs(PI/180*float(info['FlightPitch']))
    Psi = abs(PI/180*float(info['FlightYaw']))
    Theta = abs(PI/180*float(info['FlightRoll']))
    alpha = abs(PI/180*float(info['CameraYaw']))
    beta = abs(PI/180*float(info['CameraPitch']))
    ...
    print('object in G：\n')
    print(A_G)
    True_value_G = CtoG(117.6387256/180*PI,24.47122588/180*PI,50,a,e1)#原来是50
    print('True_value_G:\n')
    print(True_value_G)
    print('Error:')
    print(A_G-True_value_G)
    print(GtoC(True_value_G,e1,e2,a,b))
    return G_C
