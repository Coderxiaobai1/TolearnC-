...
def ALL(path,m,distance):
    ...
    dx = 2.54/float(info['XResolution'].split('/',1)[0])*0.01
    dy = 2.54/float(info['YResolution'].split('/',1)[0])*0.01
    ...
    return G_C, info
m = np.array([[(2919+2980)/2.0000000],[1546],[1.0000000]],dtype=np.float_)#原先设置的为杆塔顶点
#m = np.array([[(2919+2980)/2.0000000],[1743],[1.0000000]],dtype=np.float_)#现在设置为杆塔中点
